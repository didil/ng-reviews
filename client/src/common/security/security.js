  ...
  .factory('security', ['$http', '$q', '$location', 'securityRetryQueue', '$modal', function ($http, $q, $location, queue, $modal) {
    ...
    var service = {
      ...
      login: function (email, password) {
        var request = $http.post('/users/sign_in.json', {user: {email: email, password: password}});
      ...
      requestCurrentUser: function () {
        if (service.isAuthenticated()) {
          ...
          // post to new session with ignoreAuth to get user if existing while bypassing the interceptor's 401
          return $http.post('/users/sign_in.json', {}, {ignoreAuth: true}).then(function (response) {
            service.currentUser = response.data;
